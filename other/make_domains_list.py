import re
import sys
import typing


def get_values(data: typing.Dict[str, typing.Union[str, dict]]):
    for value in data.values():
        if type(value) is str:
            yield value
        else:
            yield from get_values(value)


def main(files: typing.List[str]):
    domains = set()
    for filename in files:
        print('Processing', filename)
        found = len(domains)
        with open(filename, 'r') as f:
            for line in f:
                for argument in line.strip().split(';'):
                    if argument.startswith('url='):
                        value = argument.split('=', 1)[1]
                        if value.count('/') == 2:
                            value += '/'
                        result = re.search(r'^https?://(?:www2?\.)([^/%]+)/', value)
                        if result:
                            result = result.group(1)
                            result = result.strip('.')
                            while '..' in result:
                                result = result.replace('..', '.')
                            result = str(result.encode('idna'), 'utf8')
                            domains.add(result.lower())
                        break
        print('Found', len(domains)-found)
    print('Total', len(domains))
    #domains_normalised = {}
    #for domain in domains:
    #    domain_parts = domain.split('.')
    #    domain_parts.reverse()
    #    domain_parts.append(None)
    #    current = domains_normalised
    #    for next_part, part in zip(domain_parts[1:], domain_parts):
    #        if next_part is None:
    #            current[part] = domain
    #        if part not in current:
    #            current[part] = {}
    #        current = current[part]
    #        if type(current) is str:
    #            break
    #domains = set(get_values(domains_normalised))
    #print('Total unique', len(domains))
    with open('static-extract-outlinks-domains.txt', 'w') as f:
        f.write('\n'.join(sorted(domains)))
    

if __name__ == '__main__':
    default_files = [
        '900_50states-com_news.txt',
        '900_abyz.txt',
        '900_arxiv.txt',
        '900_bbc_mediaguide.txt',
        '900_brave.txt',
        '900_einpresswire_com.txt',
        '900_gdelt_domains.txt',
        '900_github_CHOUAKIsalim_US_News_Outlets_Dataset.txt',
        '900_github_Rybatter50-cloud_Feeds.txt',
        '900_github_ercexpo_us-news-domains.txt',
        '900_github_kagisearch_kite-public.txt',
        '900_github_kotartemiy_newscatcher.txt',
        '900_github_sTechLab_local-news-dataset.txt',
        '900_github_several27_FakeNewsCorpus.txt',
        '900_github_wm-newslab_3DLNews.txt',
        '900_github_yang3kc_list_of_ethnic_media.txt',
        '900_github_yang3kc_list_of_news_domains.txt',
        '900_government.txt',
        '900_gov_uk_domains.txt',
        '900_infini_news.txt',
        '900_indymedia.txt',
        '900_localnews_SE.txt',
        '900_newspaperindex-com.txt',
        '900_onlinenewspapers-com.txt',
        '900_others.txt',
        '900_postmedia_com.txt',
        '900_russia.txt',
        '900_wikidata_Q11030_journalism.wikidata.txt',
        '900_wikidata_Q11032_newspaper.wikidata.txt',
        '900_wikidata_Q1193236_news-media.wikidata.txt',
        '900_wikidata_Q1962634_news-broadcasting.wikidata.txt',
        '900_wikidata_string_news.txt',
        '900_world-newspapers-com.txt',
        '3600_github_gabefair_News-and-Culture-Websites.txt',
        '3600_govinfo.txt',
        '3600_infini_sports.txt',
        '3600_infini_technology.txt',
        '3600_medical_research.txt',
        '3600_news_crypto_sites.txt',
        '3600_others.txt',
        '3600_ukraine-war-stats.txt',
        '3600_wikidata_Q1331793_media-company.wikidata.txt',
        '43200_abkhazia.txt',
        '43200_albania.txt',
        '43200_algeria.txt',
        '43200_andalusia.txt',
        '43200_antigua+barbuda.txt',
        '43200_armenia.txt',
        '43200_australia+oceania.txt',
        '43200_bahamas.txt',
        '43200_bangladesh.txt',
        '43200_barbados.txt',
        '43200_belarus.txt',
        '43200_big-tech.txt',
        '43200_bulgaria.txt',
        '43200_burkina-faso.txt',
        '43200_ccc+hackspaces.txt',
        '43200_china.txt',
        '43200_colombia.txt',
        '43200_cuba.txt',
        '43200_cyprus.txt',
        '43200_demoscene.txt',
        '43200_denmark.txt',
        '43200_ethiopia.txt',
        '43200_eu.txt',
        '43200_fragdenstaat_de.txt',
        '43200_georgia.txt',
        '43200_germany-2026-local-elections.txt',
        '43200_github_GSA_govt-urls.txt',
        '43200_gov2.txt',
        '43200_gov3.txt',
        '43200_gov_africa-misc.txt',
        '43200_gov_americas-misc.txt',
        '43200_gov_asia-misc.txt',
        '43200_gov_domains.txt',
        '43200_gov_europe-misc.txt',
        '43200_gov_germany.txt',
        '43200_gov_germany-regional.txt',
        '43200_gov_usa.txt',
        '43200_govt_urls.txt',
        '43200_greece.txt',
        '43200_greenland.txt',
        '43200_hong-kong.txt',
        '43200_hungary.txt',
        '43200_iceland.txt',
        '43200_india.txt',
        '43200_infini_culture.txt',
        '43200_infini_education-science.txt',
        '43200_infini_government.txt',
        '43200_infini_legal-public-safety.txt',
        '43200_infini_medical.txt',
        '43200_infini_nonprofit-religion.txt',
        '43200_infini_periodical-publishing.txt',
        '43200_infini_politics.txt',
        '43200_israel.txt',
        '43200_iran.txt',
        '43200_japan.txt',
        '43200_kazakhstan.txt',
        '43200_kosovo.txt',
        '43200_kurdistan.txt',
        '43200_laos.txt',
        '43200_lebanon.txt',
        '43200_macau.txt',
        '43200_maldives.txt',
        '43200_mali.txt',
        '43200_mil.txt',
        '43200_malaysia.txt',
        '43200_moldova.txt',
        '43200_myanmar.txt',
        '43200_nepal.txt',
        '43200_nicaragua.txt',
        '43200_niger.txt',
        '43200_palestine.txt',
        '43200_philippines.txt',
        '43200_poland.txt',
        '43200_politics_africa-misc.txt',
        '43200_politics_americas-misc.txt',
        '43200_politics_asia-misc.txt',
        '43200_politics_europe-misc.txt',
        '43200_politics_germany.txt',
        '43200_politics_other.txt',
        '43200_portugal.txt',
        '43200_romania.txt',
        '43200_russia.txt',
        '43200_sao-tome-and-principe.txt',
        '43200_slovakia.txt',
        '43200_slovenia.txt',
        '43200_south-korea.txt',
        '43200_sweden.txt',
        '43200_syria.txt',
        '43200_taiwan.txt',
        '43200_thailand.txt',
        '43200_turkey.txt',
        '43200_transnistria.txt',
        '43200_uganda.txt',
        '43200_ukraine.txt',
        '43200_vatican.txt',
        '43200_venezuela.txt',
        '43200_vietnam.txt',
        '43200_wikidata_Q7188_government.wikidata.txt',
        '43200_wikidata_Q163740_nonprofit-organization.wikidata.txt',
        '43200_wikidata_Q178706_institution.wikidata.txt',
        '43200_wikidata_Q1002697_periodical.wikidata.txt',
        '43200_wikidata_Q1331793_media-company.wikidata.txt',
        '43200_wikidata_Q3972943_publishing.wikidata.txt',
        '43200_wikidata_Q5341295_educational_organization.wikidata.txt',
        '43200_wikidata_Q5588651_governing-body.wikidata.txt',
        '43200_wikidata_Q7210356_political-organization.wikidata.txt',
        '43200_wikidata_Q16519632_scientific-organization.wikidata.txt',
        '43200_wikidata_Q101542346_cultural-facility.wikidata.txt',
        '43200_wikidata_Q105044823_publisher.wikidata.txt',
        '43200_websiteregister_rijksoverheid.txt',
        '43200_yemen.txt',
        '43200_zambia.txt',
        '86400_defense.txt',
        '86400_indivisible.org_fiftyfifty.one_affiliates.txt',
        '86400_nolympia.txt',
        '86400_others.txt',
        '86400_sports.txt',
        '86400_ukraine-war-stats.txt',
        '172800_arts+culture.txt',
        '604800_arts+culture.txt',
        '604800_ccc.txt',
        '604800_others.txt',
        '604800_sports.txt'
    ]
    files = sys.argv[1:]
    if len(files) == 0:
        files = default_files
    main(files)
